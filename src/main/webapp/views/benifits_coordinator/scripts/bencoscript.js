//USING AJAX TO PULL INFO FROM FRONT CONTROLLER SERVLET

      function getAllSuperForms(){
    		let url = "/P1Servlet/creativeconcepts/reimburesmentforms/benco_username";
	      ...
	      function getAllEmpForms(){
	  		let url = "/P1Servlet/creativeconcepts/reimburesmentforms";
	  		let xhr = new XMLHttpRequest();
	  		...
	  		xhr.onreadystatechange = function(){
	  			if(xhr.status === 200 && xhr.readyState === 4){
	  				...
	  				for(let f of forms){
	  					...
	  					fDescription.innerHTML = f["description"];
	  					fCost.innerHTML = f["cost"];
	  					fOptions.innerHTML = ["<div class='btn-group'><button type='button' class='btn btn-outline-success btn-sm' id='approved_by_super'>Approve</button> <button type='button' class='btn btn-outline-secondary btn-sm' id='view-form'>View</button> <button type='button' class='btn btn-outline-danger btn-sm' id='super-deny-form'>Deny</button> </div>"];
